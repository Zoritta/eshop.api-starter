...
public class AddressRepository(DataContext context) : IAddressRepository
{
  ...
  {
    var exists = await _context.AddressTypes.FirstOrDefaultAsync(c => c.Value.ToLower().Trim() == type.ToLower().Trim());

    if (exists is not null) throw new EShopException($"Address typen finns redan");
